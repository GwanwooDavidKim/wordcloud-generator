...
OUTPUT_DIR = 'output'
OUTPUT_FILE = os.path.join(OUTPUT_DIR, 'network_viz_style_revised.png') # 출력 파일 이름 변경

# --- 시각화 조정 변수 ---
NUM_COLS = 2
MAX_NODE_SIZE = 4500 # 최대 노드 크기 증가
MIN_NODE_SIZE = 700  # 최소 노드 크기 감소
MAX_NODE_ALPHA = 1.0 # 투명도는 최대로 고정 (색상 변화로 구분)
# MIN_NODE_ALPHA = 0.4 # 알파 변화를 원하면 이 값을 조절
FONT_SIZE = 8
FONT_WEIGHT = 'bold'
LAYOUT_K = 0.9
...
EDGE_WIDTH = 0.4 # 엣지 두께 약간 감소
EDGE_ALPHA = 0.1 # 엣지 투명도 약간 감소

# --- 2x2 레이아웃용 기본 색상 (4개) ---
# Matplotlib v1 기본 색상 또는 원하는 색상 지정 가능
PLOT_BASE_COLORS = ['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728'] # 파랑, 주황, 초록, 빨강
DEFAULT_COLOR = 'grey'

# --- 유틸리티 함수 ---
def get_color_shade_tint(base_color_rgb, importance_ratio):
    """중요도(0=max, 1=min)에 따라 기본 색상의 명암/채도 조절"""
    base_hsv = mcolors.rgb_to_hsv(base_color_rgb)
    # 중요할수록(ratio=0) 명도 낮추고 채도 약간 높임 (진하게)
    # 덜 중요할수록(ratio=1) 명도 높이고 채도 약간 낮춤 (연하게)
    value_factor = 0.7 + 0.3 * importance_ratio # 0.7(진함) ~ 1.0(기본밝기)
    saturation_factor = 1.1 - 0.2 * importance_ratio # 1.1(진함) ~ 0.9(연함)

    modified_hsv = base_hsv * [1, saturation_factor, value_factor]
    modified_rgb = mcolors.hsv_to_rgb(np.clip(modified_hsv, 0, 1))
    return tuple(modified_rgb)

def get_text_color_for_bg(bg_color_rgb):
    """배경색 밝기(Luminance) 기준으로 텍스트 색상 결정"""
...
def create_and_draw_subplots(sub_category_data):
    """하위 카테고리별 네트워크 그래프 그리기 (2x2, 기본색+명암, 중요도, 텍스트색상 반영)"""
    ...
    for sub_category, keywords in sub_category_data.items():
        ...
        # --- 기본 색상 결정 ---
        base_color_name = PLOT_BASE_COLORS[plot_index % len(PLOT_BASE_COLORS)]
        try:
        ...
        for kw_idx, keyword in enumerate(keywords):
            ...
            importance_ratio_rev = 1.0 - importance_ratio # 반대 비율 (크기/알파용)

            # 크기
            current_size = MIN_NODE_SIZE + (MAX_NODE_SIZE - MIN_NODE_SIZE) * importance_ratio_rev
            # 색상 (기본색 기준 명암 조절)
            current_color_rgb = get_color_shade_tint(base_rgb, importance_ratio)
            # 텍스트 색상
        ...
        node_list = list(G_sub.nodes()) # 레이아웃 순서와 맞추기 위해
        ...
        nx.draw_networkx_edges(G_sub, pos, ax=ax, width=EDGE_WIDTH, alpha=EDGE_ALPHA, edge_color='lightgrey') # 엣지 색상 연하게

        # 6. 레이블 개별적으로 그리기 (색상 적용 위해)
        for node, (x, y) in pos.items():
            attributes = node_attributes.get(node, {})
            text_color = attributes.get('text_color', 'black')
            # 노드 크기에 따라 폰트 크기 약간 조절 (선택적)
            # current_font_size = FONT_SIZE * math.sqrt(attributes.get('size', MIN_NODE_SIZE) / MIN_NODE_SIZE)
            # current_font_size = max(5, min(FONT_SIZE + 2, current_font_size)) # 최소/최대 크기 제한
            current_font_size = FONT_SIZE # 일단 고정
